...
class TestSessionTokenMiddleware:
    ...
    async def test_valid_token_passes(self):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get("/api/v1/things", headers={"X-Momodoc-Token": "secret-token"})
    ...
    async def test_wrong_token_returns_401(self):
        app = _make_app("secret-token")
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get("/api/v1/things", headers={"X-Momodoc-Token": "wrong-token"})
        assert resp.status_code == 401
    ...
    async def test_empty_string_token_returns_401(self):
        app = _make_app("secret-token")
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get("/api/v1/things", headers={"X-Momodoc-Token": ""})
        assert resp.status_code == 401
    ...
    async def test_token_not_yet_set_returns_503(self):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get("/api/v1/things", headers={"X-Momodoc-Token": "anything"})
